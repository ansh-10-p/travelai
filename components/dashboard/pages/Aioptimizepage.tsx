...
const SCAN_STEPS = [
  ...
  { platform: "Booking.com", msg: "Comparing Paris hotels under ₹5k/night...",    flights: 14, hotels: 14 },
...
export const AIOptimizePage = () => {
  ...
  return (
    <div>
      <SectionHeader 
        title="AI Optimizing" 
        sub={`Finding the best ${searchParams.to} trip within ₹${searchParams.budget.toLocaleString("en-IN")}`} 
      />
        ...
        <HeroBanner>
          ...
          <div>
            <div className="flex justify-between text-xs mb-2">
              ...
              <span className={`font-bold flex-shrink-0 ml-3 ${done ? "text-emerald-400" : "text-orange-400"}`}>
                {progress}%
              </span>
            </div>
      ...
      <motion.div {...stagger(1)} className="grid grid-cols-3 gap-3 mb-5">
        <StatTile label="Platforms scanned" value={`${step < 0 ? 0 : Math.min(step + 1, 8)}`} />
      ...
      <motion.div {...stagger(2)} className="mb-5">
        <Card>
          {/* FIXED CONTRAST HERE: Forced text-white and text-slate-300 so it's always legible */}
          <div className="text-sm font-semibold text-white mb-3">Live scan feed</div>
          <div className="text-xs font-mono leading-loose overflow-y-auto max-h-44 space-y-0.5 text-slate-300">
            {logLines.length === 0 && <div className="text-slate-500 italic">Initialising...</div>}
            {logLines.map((l, i) => (
              <motion.div 
                key={i} 
                initial={{ opacity: 0, x: -6 }} 
                animate={{ opacity: 1, x: 0 }} 
                className={i === logLines.length - 1 ? "text-orange-400 font-semibold" : "opacity-80"}
              >
                {l}
              </motion.div>
            ))}
            ...
            <Card accent>
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Air India non-stop + Airbnb Montmartre (4.7★)</div>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-black text-orange-500">₹72,500</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">vs ₹80,000 budget</div>
                </div>
              </div>
